          ...
          document.getElementById("expense_table").innerHTML += `
              ...
              <td>
                  <a  onmouseover="reloadEditFrame();localStorage.setItem('editExpense','${
                    data[i].id
                  }~${data[i].description}~${data[i].date}~${
            data[i].amount
      ...
      if (data.transaction_history.length > 0) {
        ...
        for (i in data.transaction_history) {
          document.getElementById("transaction_table").innerHTML += `
              ...
              <td>${c}.</td>
              <td>${data.transaction_history[i].payer} <b>|</b> ${
            data.transaction_history[i].payee
          }</td>
              <td>${data.transaction_history[i].payer_fi} <b>|</b> ${
            data.transaction_history[i].payee_fi
          }</td>
...
function getDashboardInfo() {
  fetch(ip + "/api/transaction/dashboard-information", {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-type": "application/json",
